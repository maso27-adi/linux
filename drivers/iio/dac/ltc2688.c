...
#define LTC2688_DITHER_RAW_MASK			GENMASK(13, 0)
#define LTC2688_CH_CALIBBIAS_MASK		GENMASK(13, 0)
				  ...
				  u16 code)
{
	...
	if (!c->toggle_chan && input == LTC2688_INPUT_B) {
		if (code > LTC2688_DITHER_RAW_MASK)
			return -EINVAL;
			     ...
			     int val2, long info)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		return ltc2688_dac_code_write(st, chan->channel,
					      LTC2688_INPUT_A, val);
	case IIO_CHAN_INFO_CALIBBIAS:
		if (val > LTC2688_CH_CALIBBIAS_MASK)
			return -EINVAL;
...
static int ltc2688_tgp_setup(struct ltc2688_state *st, unsigned long clk_mask,
			     const struct ltc2688_dither_helper *tgp)
{
	int ret, bit;
...
static int ltc2688_channel_config(struct ltc2688_state *st)
{
	...
	fwnode_for_each_available_child_node(fwnode, child) {
		...
		if (fwnode_property_read_bool(child, "adi,toggle-mode")) {
			...
			/* clear IIO_CHAN_INFO_RAW bit */
			clear_bit(BIT(IIO_CHAN_INFO_RAW),
		...
		ret = fwnode_property_read_u32_array(child, "adi,output-range-microvolt",
						     tmp, ARRAY_SIZE(tmp));
		if (!ret) {
			span = ltc2688_span_lookup(st, tmp[0] / 1000,
						   tmp[1] / 1000);
			if (span < 0)
				return dev_err_probe(&st->spi->dev, -EINVAL,
		...
		if (!ret) {
			 ...
			 * Hence, we prepare our TGPx <-> channel map to make it
