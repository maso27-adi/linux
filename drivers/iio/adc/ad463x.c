...
/* Sampling timing */
#define AD463X_FREQ_TO_PERIOD(f)	DIV_ROUND_UP(USEC_PER_SEC, (f / NSEC_PER_USEC))
...
#define AD4630_24_CH_MASK_SEPARATE(_realbits)					\
	((_realbits == 8) || (_realbits == 32)) ?				\
		0 : BIT(IIO_CHAN_INFO_HARDWAREGAIN) | BIT(IIO_CHAN_INFO_OFFSET)
...
static int ad463x_spi_read_reg(struct ad463x_state *st, unsigned int reg_addr,
...
static int ad463x_spi_write_reg(struct ad463x_state *st, unsigned int reg_addr,
				unsigned int reg_data)
{
	struct spi_transfer xfer = {
		.tx_buf = st->adc_data[0].buff,
...
static int ad463x_spi_write_reg_masked(struct ad463x_state *st, unsigned int reg_addr,
...
static int ad463x_reg_access(struct iio_dev *indio_dev, unsigned int reg,
...
static int ad463x_set_reg_access(struct ad463x_state *st, bool state)
{
	...
	if (state)
		...
		return ad463x_spi_write_reg(st, AD463X_REG_EXIT_CFG_MODE, AD463X_EXIT_CFG_MODE);
}

static ssize_t ad463x_sampling_freq_avail(struct device *dev, struct device_attribute *attr,
					  char *buf)
{
	ssize_t len = 0;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(ad463x_sampling_rates); i++)
		len += scnprintf(buf + len, PAGE_SIZE - len, "%d ", ad463x_sampling_rates[i]);
	buf[len - 1] = '\n';
...
static int ad463x_set_sampling_freq(struct ad463x_state *st, unsigned int freq)
{
	struct pwm_state conversion_state = {
		.period = AD463X_FREQ_TO_PERIOD(freq),
		.duty_cycle = AD463X_TRIGGER_PULSE_WIDTH_NS,
		.offset = 0,
		.enabled = true,
	}, spi_trigger_state = {
		.period = AD463X_FREQ_TO_PERIOD(freq),
...
static int ad463x_get_avg_frame_len(struct iio_dev *dev, const struct iio_chan_spec *chan)
{
...
static int ad463x_set_avg_frame_len(struct iio_dev *dev, const struct iio_chan_spec *chan,
				    unsigned int avg_len)
{
	int ret;
	struct ad463x_state *st = iio_priv(dev);
...
static int ad463x_get_pwr_mode(struct iio_dev *dev, const struct iio_chan_spec *chan)
{
	struct ad463x_state *st = iio_priv(dev);
	unsigned int mode;
	int ret;

	ret = ad463x_spi_read_reg(st, AD463X_REG_DEVICE_CONFIG, &mode);
	if (ret < 0)
		return ret;
...
static int ad463x_set_pwr_mode(struct iio_dev *dev, const struct iio_chan_spec *chan,
			       unsigned int mode)
{
	struct ad463x_state *st = iio_priv(dev);
...
static int ad463x_phy_init(struct ad463x_state *st)
{
	...
	ret = ad463x_spi_write_reg_masked(st, AD463X_REG_MODES, AD463X_LANE_MODE_MSK,
					  st->phy.lane_mode);
	if (ret < 0)
		return ret;
	ret = ad463x_spi_write_reg_masked(st, AD463X_REG_MODES, AD463X_CLK_MODE_MSK,
					  st->phy.clock_mode);
	if (ret < 0)
		return ret;
	ret = ad463x_spi_write_reg_masked(st, AD463X_REG_MODES, AD463X_DATA_RATE_MODE_MSK,
					  st->phy.data_rate_mode);
	if (ret < 0)
		return ret;
	ret = ad463x_spi_write_reg_masked(st, AD463X_REG_MODES, AD463X_OUT_DATA_MODE_MSK,
					  st->phy.out_data_mode);
	if (ret < 0)
		return ret;

	if (st->phy.out_data_mode == AD463X_30_AVERAGED_DIFF) {
		ret = ad463x_spi_write_reg(st, AD463X_REG_AVG, AD463X_AVG_LEN_DEFAULT);
		if (ret < 0)
			return ret;
...
static int ad463x_get_string_index(const char *string, const char *const *string_array,
...
static int ad463x_parse_dt(struct ad463x_state *st)
{
	...
	ret = fwnode_property_read_string(fwnode, "adi,lane-mode", &lane_mode);
	if (ret < 0)
		return ret;
	ret = fwnode_property_read_string(fwnode, "adi,clock-mode", &clock_mode);
	if (ret < 0)
		return ret;
	ret = fwnode_property_read_string(fwnode, "adi,data-rate-mode", &data_rate_mode);
	if (ret < 0)
		return ret;
	ret = fwnode_property_read_string(fwnode, "adi,out-data-mode", &out_data_mode);
	if (ret < 0)
		return ret;
...
static int ad463x_set_chan_offset(struct ad463x_state *st, int chan_idx, int offset)
{
	int ret;
...
static int ad463x_read_raw(struct iio_dev *indio_dev, struct iio_chan_spec const *chan, int *val,
			   int *val2, long info)
{
	struct ad463x_state *st = iio_priv(indio_dev);
	unsigned int temp;
...
static int ad463x_write_raw(struct iio_dev *indio_dev, struct iio_chan_spec const *chan, int val,
			    int val2, long info)
{
	int ret;
	struct ad463x_state *st = iio_priv(indio_dev);
...
static int ad463x_buffer_preenable(struct iio_dev *indio_dev)
{
	...
	xfer.bits_per_word = AD463X_SPI_WIDTH(
		st->phy.lane_mode, ad463x_data_widths[st->phy.out_data_mode]);
...
static struct iio_chan_spec_ext_info ad463x_ext_info[] = {
	...
	IIO_ENUM_AVAILABLE_SHARED("sample_averaging", IIO_SHARED_BY_ALL,
				  &ad463x_avg_frame_len_enum),
	IIO_ENUM("operating_mode", IIO_SHARED_BY_ALL, &ad463x_power_mode_enum),
...
static int ad463x_probe(struct spi_device *spi)
{
	...
	ret = devm_add_action_or_reset(&spi->dev, ad463x_avg_trigger_disable,
	...
	buffer = devm_iio_dmaengine_buffer_alloc(
		indio_dev->dev.parent, "rx", &ad463x_dma_buffer_ops, indio_dev);
	if (IS_ERR(buffer))
